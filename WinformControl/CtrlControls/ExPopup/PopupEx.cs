...
namespace CtrlControls.ExPopup
{
    ...
    /// </summary>
    [ToolboxItem(false)]
    public partial class PopupEx : ToolStripDropDown
    {
        ...
        private PopupEx ownerPopup;
        private PopupEx childPopup;
        ...
        /// <remarks>
        /// Pop-up will be disposed immediately after disposion of the content MyControl.
        /// </remarks>
        /// <exception cref="T:System.ArgumentNullException"><paramref name="content" /> is <code>null</code>.</exception>
        public PopupEx(Control content)
        {
            if (content == null)
            {
        ...
        /// <returns>
        /// true if the key was processed by the MyControl; otherwise, false.
        /// </returns>
        ...
        /// <summary>
        /// Shows the pop-up window below the specified MyControl.
        /// </summary>
        /// <param name="MyControl">The MyControl below which the pop-up will be shown.</param>
        /// <remarks>
        /// When there is no space below the specified MyControl, the pop-up MyControl is shown above it.
        /// </remarks>
        /// <exception cref="T:System.ArgumentNullException"><paramref name="MyControl"/> is <code>null</code>.</exception>
        public void Show(Control MyControl)
        {
            Show(MyControl, MyControl.ClientRectangle);
        }

        public void Show(Control MyControl, bool center)
        {
            Show(MyControl, MyControl.ClientRectangle, center);
        }

        /// <summary>
        /// Shows the pop-up window below the specified area of the specified MyControl.
        /// </summary>
        /// <param name="MyControl">The MyControl used to compute screen location of specified area.</param>
        /// <param name="area">The area of MyControl below which the pop-up will be shown.</param>
        /// <remarks>
        /// When there is no space below specified area, the pop-up MyControl is shown above it.
        /// </remarks>
        /// <exception cref="T:System.ArgumentNullException"><paramref name="MyControl"/> is <code>null</code>.</exception>
        public void Show(Control MyControl, Rectangle area)
        {
            Show(MyControl, area, false);
        }

        public void Show(Control MyControl, Rectangle area, bool center)
        {
            if (MyControl == null)
            {
                throw new ArgumentNullException("MyControl");
            }
            SetOwnerItem(MyControl);
            resizableTop = resizableLeft = false;
            Point location = MyControl.PointToScreen(new Point(area.Left, area.Top + area.Height));
            ...
            location = MyControl.PointToClient(location);
            Show(MyControl, location, ToolStripDropDownDirection.BelowRight);
        }
        ...
        private void SetOwnerItem(Control control)
        {
            ...
            if (control is PopupEx)
            {
                PopupEx popupControl = control as PopupEx;
        ...
        /// <summary>
        /// Raises the <see cref="E:System.Windows.Forms.MyControl.SizeChanged" /> event.
        /// </summary>
        ...
        {
            if (m.Msg == (int)WindowsMessage.WM_NCACTIVATE && m.WParam != IntPtr.Zero && childPopup != null && childPopup.Visible)
            {
                childPopup.Hide();
            }
            if (!this.Resizable)
            ...
            else if (m.Msg == (int)WindowsMessage.WM_GETMINMAXINFO)
            {
        ...
        {
            int x = Win32Helper.LOWORD(m.LParam);
            int y = Win32Helper.HIWORD(m.LParam);
            Point clientLocation = base.PointToClient(new Point(x, y));
            ...
            if (resizableTop)
            {
                ...
                {
                    m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTTOPLEFT;
                    return true;
                }
                if (!resizableLeft && gripBouns.TopRight.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTTOPRIGHT;
                    return true;
                }
                if (gripBouns.Top.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTTOP;
                    return true;
                }
            }
            else
            {
                ...
                {
                    m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTBOTTOMLEFT;
                    return true;
                }
                if (!resizableLeft && gripBouns.BottomRight.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTBOTTOMRIGHT;
                    return true;
                }
                if (gripBouns.Bottom.Contains(clientLocation))
                {
                    m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTBOTTOM;
                    return true;
                }
            }
            if (resizableLeft && gripBouns.Left.Contains(clientLocation))
            {
                m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTLEFT;
                return true;
            }
            if (!resizableLeft && gripBouns.Right.Contains(clientLocation))
            {
                m.Result = contentControl ? transparent : (IntPtr)DefWindowProc.HTRIGHT;
                return true;
            }
            return false;
        }
